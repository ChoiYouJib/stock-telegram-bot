...
def get_market_status():
    ...
    kor_stat = "휴장" if today in HOLIDAYS_KOR else ("개장 중" if 9 <= now.hour < 15 else ("장 시작 전" if now.hour < 9 else "장 종료"))
    usa_stat = "휴장" if today in HOLIDAYS_USA else ("개장 중" if (now.hour >= 22 or now.hour < 5) else ("장 시작 전" if 5 <= now.hour < 22 else "장 종료"))
...
def get_news(now):
    # 한국 시간 22시~06시 사이면 미국 뉴스(US), 아니면 한국 뉴스(KR)
    is_night = (now.hour >= 22 or now.hour < 6)
    url = "https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=en-US&gl=US&ceid=US:en" if is_night else "https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=ko&gl=KR&ceid=KR:ko"
    try:
...
def get_market_data():
    ...
    msg += f"\n📅 {now.month}월 휴장 예정일\n- {MONTHLY_HOLIDAYS.get(now.month, '없음')}\n"
    msg += "\n📰 시장 주요 이슈\n" + get_news(now)
    return msg
